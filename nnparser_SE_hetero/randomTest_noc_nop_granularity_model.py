...
if __name__ == '__main__':
	...
	for line in lines:
		if line.startswith("#"):
			...
			if layer_name == str(sys.argv[2]) and line_item[1] == "CONV":
				network_param = {}
				...
				filename = './randomTest_result_'+DNN_model+'/'+layer_name+'_granularity'+'.xls'
				
				PE_C_list = [8,16]
				PE_K_list = [8,16]
				core_num_list = [4,8,16,64]
							...
							for chiplet_num in chiplet_num_list:
								for l1 in l1_list:
									...
									if valid(PE_C,PE_K,core_num,chiplet_num,WL1,AL1,OL1,WL2,AL2,OL2):
										...
										iterTime = 10000
										result_list,edp_res_min = randomTest(GATest, iterTime, HW_param, memory_param, if_multicast, filename)
										...
										result_list.append(area_chip_mm2 * chiplet_num * edp_res_min)
